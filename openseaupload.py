...
def main_program_loop():
    ...
    while end_num >= start_num:
        print("Start creating NFT " + str(arr[start_num].split(".jp")[0]) + str(start_num))
        ...
        imagePath = os.path.abspath(file_path + "\\" + str(arr[start_num].split(".jp")[0]) + "." + loop_file_format)  # change folder here
        imageUpload.send_keys(imagePath)

        name = driver.find_element_by_xpath('//*[@id="name"]')
        print(str(arr[start_num].split(".j")[0]))
        name.send_keys(str(arr[start_num].split(".j")[0]) + str(start_num))  # +1000 for other folders #change name before "#"
        time.sleep(0.5)
        ...
        if is_polygon.get():
            blockchain_button = driver.find_element(By.XPATH, '//*[@id="__next"]/div[1]/main/div/div/section/div/form/div[7]/div/div[2]')
            blockchain_button.click()
            polygon_button_location = 'sspan[normalize-space() = "Mumbai"]'
            #spolygon_button_location = '//*[@id="tippy-14549"]/div/div/div/ul/li/button'
            ...
            print("button pressed : "+ polygon_button_location)
